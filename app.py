...
with st.expander("📖 Guia Rápido: Como preparar a sua planilha"):
    ...
    * **📁 Formatos Aceitos:** Arquivos **Excel (`.xlsx`)** ou **CSV (`.csv`)**. O sistema detecta o separador automaticamente.
    * **📋 Cabeçalho:** A tabela deve possuir apenas **uma linha de cabeçalho** (nomes das colunas). Evite células mescladas.
    * **📅 Datas:** Devem estar em formato reconhecível (ex: `DD/MM/AAAA`). O sistema converte automaticamente o mês para **algarismos romanos** (ex: `15/03/2024` → `15.iii.2024`).
...
arquivo_upload = st.file_uploader("Arraste ou escolha a planilha Excel (.xlsx ou .csv)", type=["xlsx", "csv"], help="Para arquivos CSV, o separador é detectado automaticamente.")
